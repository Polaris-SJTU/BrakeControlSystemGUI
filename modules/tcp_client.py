...
class RunningMode(enum.IntEnum):
    FIELD_CONTROL = 0b01 # 现场控制 (主要用于上行)
...
# --- TcpClient 父类 (新增 log_message 信号) ---
class TcpClient(QObject):
    ...
    def __init__(self, host: str, port: int):
        ...
        self.log_message.emit(f"TcpClient: Initializing for {self._host}:{self._port}. Auto-connecting...")
    ...
    def _connect_to_server(self):
        ...
        if self.socket.state() == QAbstractSocket.UnconnectedState:
            self.log_message.emit(f"TcpClient: Attempting to connect to {self._host}:{self._port}...")
            self.socket.connectToHost(self._host, self._port)
        elif self.socket.state() == QAbstractSocket.ConnectingState:
            self.log_message.emit(f"TcpClient: Already connecting to {self._host}:{self._port}...")
        # else: 连接中或已连接，不做任何操作

    def _on_connected(self):
        """处理连接成功事件。"""
        self.log_message.emit(f"TcpClient: Successfully connected to {self._host}:{self._port}")
        self._reconnect_timer.stop() # 连接成功，停止重连定时器

    def _on_disconnected(self):
        """处理断开连接事件。"""
        self.log_message.emit(f"TcpClient: Disconnected from {self._host}:{self._port}. Scheduling reconnect...")
    ...
    def _on_socket_error(self, error):
        ...
        self.log_message.emit(f"TcpClient: Socket error from {self._host}:{self._port}: {error_str} ({error})")
        self._schedule_reconnect()

    def _schedule_reconnect(self):
        """调度重连尝试。"""
        if not self._reconnect_timer.isActive():
            self.log_message.emit(f"TcpClient: Reconnecting in {self._reconnect_interval / 1000} seconds...")
    ...
    def _do_send_data(self, data: bytes):
        ...
        if self.socket.state() == QAbstractSocket.ConnectedState:
            ...
            if bytes_written < 0:
                self.log_message.emit(f"TcpClient: Error writing data: {self.socket.errorString()}")
            else:
                self.log_message.emit(f"TcpClient: Sent {bytes_written} bytes: {data.hex().upper()}")
                # 检查是否完全写入，虽然write()通常会全部写入，但在某些情况下可能不会
                if bytes_written != len(data):
                    self.log_message.emit(f"TcpClient: Warning: Only {bytes_written}/{len(data)} bytes written for {data.hex().upper()}")
        else:
            self.log_message.emit(f"TcpClient: Failed to send data, not connected. Data: {data.hex().upper()}")
    ...
    def _on_ready_read(self):
        ...
        while self.socket.bytesAvailable():
            ...
            if data:
                self.log_message.emit(f"TcpClient: Received {len(data)} bytes: {data.hex().upper()}")
    ...
    def shutdown(self):
        ...
        """
        self.log_message.emit(f"TcpClient: Shutting down client for {self._host}:{self._port}...")
...
class DownlinkTcpClient(TcpClient):
    ...
    def __init__(self, host: str, port: int):
        ...
        self.log_message.emit(f"DownlinkTcpClient: Initialized for {host}:{port}.")
        ...
        for byte_val in raw_data: # 迭代每个字节
            ...
            if self.parsing_state == ParsingState.WAIT_HEADER:
                ...
                if byte == 0xAA:
                    ...
                    self.log_message.emit(f"Parser: Header 0xAA found. Switching to RECEIVING_BODY. Buffer: {self.current_packet_buffer.hex().upper()}")
                else:
                    # 6. 当接收到错误的byte时，回到状态机初始状态，等待新的一帧数据
                    # 丢弃非帧头字节，并保持在 WAIT_HEADER 状态，或者如果缓冲区有内容，清空
                    if len(self.current_packet_buffer) > 0:
                        self.log_message.emit(f"Parser: Discarding non-header byte {byte:02X}. Buffer was {self.current_packet_buffer.hex().upper()}. Resetting.")
                        self.reset_parser_state() # 严格来说，如果缓冲区已有内容，应该清空并重置
                    else:
                        self.log_message.emit(f"Parser: Discarding non-header byte {byte:02X} in WAIT_HEADER state (expected 0xAA).")
                ...
                if len(self.current_packet_buffer) == self.expected_uplink_length:
                    if self._validate_and_parse_uplink_packet():
                        self.log_message.emit(f"Parser: Valid uplink packet parsed: {self.current_packet_buffer.hex().upper()}")
                    else:
                        # 验证失败时也打印完整缓冲区，并重置
                        self.log_message.emit(f"Parser: Invalid uplink packet detected. Resetting parser. Buffer: {self.current_packet_buffer.hex().upper()}")
                    ...
                    self.log_message.emit(f"Parser: Buffer overflow ({len(self.current_packet_buffer)} bytes) in RECEIVING_BODY. Expected {self.expected_uplink_length}. Resetting parser. Buffer: {self.current_packet_buffer.hex().upper()}")
                    self.reset_parser_state() # 发生错误，重置状态机
                else:
                    self.log_message.emit(f"Parser: Receiving body. Current length: {len(self.current_packet_buffer)}/{self.expected_uplink_length}. Buffer: {self.current_packet_buffer.hex().upper()}")

    def reset_parser_state(self):
        """重置解析状态机到初始状态。"""
        self.current_packet_buffer = bytearray()
        self.parsing_state = ParsingState.WAIT_HEADER
        self.log_message.emit("Parser: State reset to WAIT_HEADER.")
    ...
    def _validate_and_parse_uplink_packet(self) -> bool:
        ...
        if len(packet) != self.UPLINK_PACKET_LENGTH:
            self.log_message.emit(f"Parser Validation Error: Incorrect uplink packet length ({len(packet)} vs {self.UPLINK_PACKET_LENGTH}) for {packet.hex().upper()}")
            return False

        # 2. 帧头校验 (0xAA)
        if packet[0] != 0xAA:
            self.log_message.emit(f"Parser Validation Error: Invalid uplink header byte {packet[0]:02X} (expected 0xAA) for {packet.hex().upper()}")
            return False

        # 3. DIR (通信方向) 校验 (0b01 for uplink)
        dir_val = (packet[1] >> 6) & 0b11
        if dir_val != CommunicationDirection.UPLINK.value:
            self.log_message.emit(f"Parser Validation Error: Invalid uplink DIR {dir_val:02b} (expected {CommunicationDirection.UPLINK.value:02b}) for {packet.hex().upper()}")
            return False

        # 4. 帧尾校验 (0x55)
        if packet[6] != 0x55:
            self.log_message.emit(f"Parser Validation Error: Invalid uplink tail byte {packet[6]:02X} (expected 0x55) for {packet.hex().upper()}")
            return False
        ...
        if calculated_checksum != received_checksum:
            self.log_message.emit(f"Parser Validation Error: Uplink checksum mismatch for {packet.hex().upper()}. Calculated: {calculated_checksum:02X}, Received: {received_checksum:02X}")
            return False
        ...
        try:
            ...
            self.log_message.emit(f"Parser Error: Could not convert enum value - {e}. Packet: {packet.hex().upper()}")
            return False
        except Exception as e:
            self.log_message.emit(f"Parser Unexpected Error: {e}. Packet: {packet.hex().upper()}")
            return False
    ...
    def _construct_and_send_downlink_packet(self, command_dict: dict):
        ...
        try:
            ...
            if len(packet) != self.DOWNLINK_PACKET_LENGTH:
                self.log_message.emit(f"Construct Error: Downlink packet length mismatch! Expected {self.DOWNLINK_PACKET_LENGTH}, got {len(packet)}")
                return

            self.log_message.emit(f"DownlinkTcpClient: Constructed downlink packet: {packet.hex().upper()}")
            super().send_data(bytes(packet)) # 调用父类的 send_data 方法来发送字节数据

        except KeyError as e:
            self.log_message.emit(f"DownlinkTcpClient Error: Missing or invalid enum key in command_dict: {e}. Dict: {command_dict}")
        except ValueError as e:
            self.log_message.emit(f"DownlinkTcpClient Error: Invalid value in command_dict (e.g. non-int for TRACK/DEVICE) - {e}. Dict: {command_dict}")
        except Exception as e:
            self.log_message.emit(f"DownlinkTcpClient Error constructing downlink packet: {e} for dict: {command_dict}")